...
resource "google_cloud_run_v2_service" "lep_backend" {
  ...
  template {
    ...
    containers {
      ...
      dynamic "env" {
        for_each = var.twilio_account_sid != "" ? [1] : []
        content {
          name = "TWILIO_ACCOUNT_SID"
          value_source {
            secret_key_ref {
              secret  = google_secret_manager_secret.twilio_account_sid[0].secret_id
              version = "latest"
            }
          }
        }
      ...
      dynamic "env" {
        for_each = var.twilio_auth_token != "" ? [1] : []
        content {
          name = "TWILIO_AUTH_TOKEN"
          value_source {
            secret_key_ref {
              secret  = google_secret_manager_secret.twilio_auth_token[0].secret_id
              version = "latest"
            }
          }
        }
      ...
      dynamic "env" {
        for_each = var.twilio_phone_number != "" ? [1] : []
        content {
          name = "TWILIO_PHONE_NUMBER"
          value_source {
            secret_key_ref {
              secret  = google_secret_manager_secret.twilio_phone_number[0].secret_id
              version = "latest"
            }
          }
        }
      ...
      dynamic "env" {
        ...
        content {
          ...
          value_source {
            secret_key_ref {
              secret  = google_secret_manager_secret.smtp_username[0].secret_id
              version = "latest"
            }
          }
        }
